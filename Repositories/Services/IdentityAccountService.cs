    ...
    public class IdentityAccountService : IUser
    {
        ...
        public async Task<UserDTO> Login(string name, string password)
        {
            ...
            var result = new UserDTO()
            {
                ...
                Token = await _jwtTokenService.GenerateToken(user, System.TimeSpan.FromMinutes(7))
            };
            return result;
        }    
        ...
        public async Task<UserDTO> GetToken(ClaimsPrincipal claimsPrincipal)
        {
            ...
            {
                UserName = user.UserName,
                Token = await _jwtTokenService.GenerateToken(user, System.TimeSpan.FromMinutes(1))
            };
        }      
        public async Task<List<string>> ViewAnnouncements()
        {
          var allAnnoun = await _context.Announcement.Select(a => a.Title + a.Content + a.CreatedTime).ToListAsync();
          return allAnnoun;
        }
